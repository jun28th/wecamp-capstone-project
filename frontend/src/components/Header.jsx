        ...
        <div className="flex flex-1 justify-center gap-0 sm:gap-1">
          {NAV_LINKS.map((link) => (
            <NavLink
              ...
              data-od-id={link.id}
              className={({ isActive }) =>
                `flex items-center min-h-[44px] px-2.5 sm:px-4 py-2 rounded-[var(--radius-input)] font-[var(--font-body)] text-[13px] sm:text-[15px] font-semibold text-[var(--color-ink)] no-underline transition-all duration-150 ease-out hover:bg-[var(--color-surface-alt)] hover:text-[var(--color-primary-deep)]
                ${isActive ? "bg-[var(--color-surface-alt)] text-[var(--color-primary-deep)] shadow-[var(--shadow-1)]" : ""}`
              }
            >
              {link.label}
            </NavLink>
